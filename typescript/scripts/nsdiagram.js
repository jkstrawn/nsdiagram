...
Box.prototype.update = function () {
    if(this.incrementY != 0) {
        ...
        this.incrementY = Math.floor(Math.pow(this.incrementY * signY, 0.95)) * signY;
    ...
    if(this.incrementX != 0) {
        ...
        this.incrementX = Math.floor(Math.pow(this.incrementX * signX, 0.95)) * signX;
...
Block.prototype.createTextDiv = function (parent, x, y, text, width, writable, label, bottom) {
    ...
    var inputDiv = "<div class='texts' id='text" + id + "' style='position:absolute; z-index:3;'>" + "<textarea type='text' readonly='readonly' onblur='elementBlur(this);'" + " onkeydown='inputFieldKey(event, this)' style='width:" + (this.width - 2) + "px;'>" + text + "</textarea>" + "</div>";
...
ContainerBlock.prototype.init = function (container, makeBefore) {
    ...
    this.widthOfLine = 0.6;
    Block.prototype.init.call(this, container, makeBefore);
    this.textList[0].width = this.width * 0.6;
    this.textList[0].xOffset = this.width * 0.2;
...
CaseBlock.prototype.init = function (container, makeBefore) {
    ...
    Block.prototype.init.call(this, container, makeBefore);
    this.textList[0].xOffset = this.width * 0.21;
    this.textList[0].width = this.width * 0.69;
    var div = this.textList[0].link;
    var inputs = div.getElementsByTagName('textarea');
    var input = inputs[0];
...
CaseBlock.prototype.addCase = function () {
    ...
    var container = new Container(this.app, this, this.x + (numOfCases - 1) * width, this.y + this.childOffsetY + (this.numOfLinesBottom - 1 + this.numOfLines - 1) * this.app.buttonHeight, width + odd, this.getLargestContainer());
...
Button.prototype.createLabel = function (id, labelText, x, y) {
    var newLabelDiv = "<div class='texts' id='label" + id + "' style='position:absolute; z-index:30; background: rgba(255, 255, 255, 0.7);'>" + "<p>" + labelText + "</p>" + "</div>";
...
ButtonList.prototype.click = function () {
    ...
    if((!selected && index < 6) || (index == 5 && selected.blockList)) {
        return;
    }
    if(index > 5) {
        index += 100;
